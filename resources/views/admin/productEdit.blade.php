...
@section('content_admin')
    <div class="d-flex flex-column justify-content-center mx-auto w-full">
        ...
        @endif
        <form class="form-add" style="width: 800px;" action="{{ route('products.handleEdit', ['id' => $productDetail->id]) }}"
            method="POST">
            @csrf
            <div class="mb-3">
                <label for="exampleInputEmail1" class="form-label">Name</label>
                <input type="text" name="name" class="form-control"
                    value="{{ old('name', $productDetail->name) ?? $productDetail->name }}" placeholder="Name...">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" placeholder="Price..."
                    value="{{ old('price') ?? $productDetail->price }}" name="price">
                @error('price')
                    <span style="color: red">
                        {{ $message }}
                    </span>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" placeholder="Description..."
                    value="{{ old('description') ?? $productDetail->description }}" name="description">
                @error('desc')
                    <span style="color: red">
                        {{ $message }}
                    </span>
            ...
            <div class="mb-3">
                ...
                <input type="file" class="form-control" placeholder="Images..."
                    value="{{ old('image') ?? $productDetail->image }}" name="image">
                @error('image')
                    <span style="color: red">
                        {{ $message }}
                    </span>
